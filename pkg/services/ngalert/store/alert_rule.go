...
func (st DBstore) filterByPluginOrigin(filter ngmodels.PluginOriginFilter, sess *xorm.Session) (*xorm.Session, error) {
	...
	case ngmodels.PluginOriginFilterHide:
		sql, args, err = buildLabelKeyMissingCondition(st.SQLStore.GetDialect(), "labels", PluginGrafanaOriginLabel)
	case ngmodels.PluginOriginFilterOnly:
		sql, args, err = buildLabelKeyExistsCondition(st.SQLStore.GetDialect(), "labels", PluginGrafanaOriginLabel)
	default:
